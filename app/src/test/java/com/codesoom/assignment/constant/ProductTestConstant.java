package com.codesoom.assignment.constant;

public class ProductTestConstant {
    public static final Long NOT_EXISTS_ID = 10000L;
    public static final String TOY_NAME = "고양이";
    public static final String TOY_MAKER = "메이커";
    public static final String IMAGE_URL = "이미지";
    public static final String CHANGE_NAME = "고양이 장난감";
    public static final String CHANGE_MAKER = "고양이 메이커";
    public static final String CHANGE_IMAGE_URL = "고양이 이미지";
    public static final int PRICE = 10000;
    public static final int CHANGE_PRICE = 9000;
}
